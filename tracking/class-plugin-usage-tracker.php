...
if( ! class_exists( 'DCMA_Plugin_Tracker') ) {
	...
	class DCMA_Plugin_Tracker {
		...
		public function force_track_for_one_time(){
			$is_tracked = get_option( 'wpins_'. $this->plugin_name .'_force_tracked', false );
			if( ! $is_tracked ) {
		...
		protected function remote_post( $url, $data = array(), $args = array() ){
		...
		public function send_data( $body ) {
			...
			if( ! empty( $old_diff_data ) && $diff_data != $old_diff_data ) {
				$old_diff_data = array_merge( $old_diff_data, $diff_data );
			}

			if( ! empty( $old_diff_data ) ) {
				$old_diff_data = array_merge(
					array(
			...
			if( empty( $diff_data ) && empty( $old_diff_data ) && $site_id == false ) {
				...
				$request = $this->remote_post( $this->home_url, $body );
				if( ! is_wp_error( $request ) && $request['response']['code'] == 200 ) {
					update_option( $this->plugin_name . '_site_data', $body );
				}
			}
		...
		public function optin_notice() {
			...
			if ( $is_local ) {
				...
				if( $this->marketing != 1 ) {
					...
					$notice_text = __( 'Want to help make <strong>ReviewX</strong> even more awesome? You can get a <strong>25% discount coupon</strong> for Premium extensions if you allow us to track the usage. <a class="wpsp-insights-data-we-collect" href="#">What we collect.</a>', 'plugin-usage-tracker' );
				}
